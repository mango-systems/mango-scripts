...
for script_file in script_files:
    ...
    # Check if the script in "validated_scripts" is different from the one in "scripts"
    if not os.path.exists(validated_script_path) or (
        os.path.getmtime(script_path) > os.path.getmtime(validated_script_path)
    ):
        # Copy the script to "validated_scripts" if it's different
        os.system(f"cp {script_path} validated_scripts/")
